...
namespace HortaIn.DAL.Services
{
    public class AzureStorage : IAzureStorage
    {
        ...
        public AzureStorage(IConfiguration configuration, ILogger<AzureStorage> logger)
        {
            ...
            _storageConnectionString = configuration.GetValue<string>("BlobConnectionString"); //verificar aqui se der pau
        ...
        public async Task<List<BlobDto>> ListByUserIdAsync(string userId)
        {
            // Get a reference to a container named in appsettings.json
            BlobContainerClient container = new BlobContainerClient(_storageConnectionString, _storageContainerName);
        ...
        public async Task<List<BlobDto>> DownloadFilesAsync(string[] blobFilenames)
        {
            // Get a reference to a container named in appsettings.json
            BlobContainerClient client = new BlobContainerClient(_storageConnectionString, _storageContainerName);
            ...
            foreach (var blobFilename in blobFilenames)
            {
                try
                {
                    ...
                    if (await file.ExistsAsync())
                    {
                        ...
                        blobs.Add(
                            new BlobDto { Content = blobContent, Name = name, ContentType = contentType }
                            );                        
                    }                    
                }
                catch (RequestFailedException ex)
